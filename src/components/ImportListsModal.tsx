...
export default function ImportListsModal({ open, onClose, onImported }: ImportListsModalProps) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (!token) {
        setError('You must be signed in to import lists.');
        return;
      }
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-zen-900">Import lists</h2>
                  <p className="text-sm text-zen-500">From a JSON file</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {!result && (
              <div className="rounded-2xl bg-zen-50 border border-zen-100 p-4 space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-zen-400">File format</p>
                <div className="space-y-2 text-sm text-zen-600">
                  <p>
                    <strong className="text-zen-800">JSON</strong> — array of list objects, each with{' '}
                    <code className="bg-zen-100 rounded px-1 text-xs">name</code> (required),{' '}
                    <code className="bg-zen-100 rounded px-1 text-xs">description</code> (optional), and{' '}
                    <code className="bg-zen-100 rounded px-1 text-xs">items</code> (optional array of{' '}
                    <code className="bg-zen-100 rounded px-1 text-xs">{'{ content, completed }'}</code>)
                  </p>
                </div>
                <pre className="rounded-xl bg-zen-100 px-3 py-2 text-xs text-zen-700 overflow-x-auto">{`[
  {
    "name": "Shopping list",
    ...
    "items": [
      ...
      { "content": "Eggs", "completed": false }
                ...
                <p className="text-xs text-zen-400">Up to 50 lists per import.</p>
              </div>
            )}
              ...
              >
                ...
                {selectedFile ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs text-zen-400">.json</p>
                  </div>
                )}
              </div>
            )}
            ...
            {result && (
              <div className="space-y-3">
                <div className="flex items-start gap-2 rounded-2xl border border-sage-200 bg-sage-50 px-4 py-3 text-sm text-sage-800">
                  ...
                  <span>
                    Imported <strong>{result.imported}</strong> list{result.imported !== 1 ? 's' : ''} successfully.
                    {result.skipped > 0 && ` ${result.skipped} skipped.`}
                    {result.truncated > 0 && ` ${result.truncated} omitted (50-list limit).`}
                  </span>
                </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-1">
              ...
              {!result && (
                ...
                >
                  {importing ? 'Importing…' : 'Import lists'}
                </button>
              )}
            </div>
          </motion.div>
