...
8. **Android / Capacitor** – The Android app's WebView serves the bundle at `https://localhost`. After Google OAuth, Supabase must redirect back to that origin so the app regains control instead of opening the public website. Add `https://localhost` to _Authentication → URL Configuration → Redirect URLs_ in your Supabase project. No changes to `NEXT_PUBLIC_SITE_URL` or the Google Cloud Console are required for this; the redirect is handled automatically when the app detects it is running as a native Capacitor build.
